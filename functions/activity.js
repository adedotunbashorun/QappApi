...
const getBody = message => {
  ...
  encodedBody = encodedBody
    ...
    .replace(/_/g, "")
    .replace(/\s/g, "");
  return striptags(base64.decode(encodedBody))
    .replace(/\n |\r/g, "")
    .replace(/\n |\n/g, "");
...
const html = data => {
  return (
    '<div id="content" style="background-color: #1D4BB7;width:100%;">' +
    "<nav>" +
    ...
    "</div>" +
    "</nav>" +
    '<div style="background-color: #fefefe;padding:20px;color:#000;">' +
    data +
    "</div>" +
    "</div>"
  );
...
const Sms = (number, message) => {
  ...
  request(options, (err, body) => {
    ...
    else console.log(body, "success");
  });
};

Activity.sendScheduleMessage = async () => {
  try {
    const schedules = await Schedule.find({ status: { $ne: true } });

    for (let i = 0; i < schedules.length; ++i) {
      let schedule = schedules[i];
      let schedule_date =
        new Date(schedule.scheduled_date).getFullYear() +
        "-" +
        (new Date(schedule.scheduled_date).getMonth() + 1) +
        "-" +
      ...
      if (schedule && schedule_date == current_date) {
        const user = await User.findOne({ _id: schedule.user_id });
        const question = await Question.findOne({
          _id: schedule.question_id
        ...
        if (user.medium == "Sms") {
          ...
          const sched = await Schedule.findOne({ _id: schedule._id });
          sched.status = true;
          sched.save();
        } else {
          Email(
            user,
            question.category_id.name,
            html(
              "Good morning " +
                user.title +
                " " +
          ...
          const saveSched = await Schedule.findOne({ _id: schedule._id });

          saveSched.status = true;
          saveSched.save();
        }
      }
    }
...
Activity.unrepliedScheduleMessage = async () => {
  try {
    const schedules = await Schedule.find({ is_reply: { $ne: true } });
    ...
    for (let i = 0; i < schedules.length; ++i) {
      ...
      if (schedule && schedule_date === current_date) {
        ...
        if (user.medium == "Sms") {
          ...
          Sms(
            user.phone,
            "If you simply forgot to email me to indicate that you had completed the task, please email me as soon as you can with your photographic evidence to let me know. If however, it completely skipped your mind, don’t worry. It is normal to forget things on occasion."
          );
          Sms(
            user.phone,
            "There will be additional opportunities to earn more ballots for the draw to win the gift card.\r\nIf, however, you no longer wish to participate in the study, please let me know as well."
          );
          Sms(
            user.phone,
